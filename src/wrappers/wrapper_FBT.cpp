...
double compute_hankel_FBT(int nu, form_factor_f intern_fct, void *ctx, double x, int n_method,
    ...
    Receives:
        ...
        - intern_fct:  function to use for computing the transform, meant to
    receive x and *ctx (see below for what these are)
        - ctx:         struct containing both the function f to hankel-transform
    and the function parameters
        ...
        - f_max:       constant corresponding to initial guess where function has
    maximum
    */

    // This is a bit of a hack, but it's needed to avoid issues when calling ogata
    // functions
