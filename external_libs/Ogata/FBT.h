...
class FBT {
  private:
    ...
    int option; // 0 - modified Ogata (default), 1 - unmodified Ogata, 2 -
                // unmodified ogata with h=0.05
    constexpr static double nu_def = 0.0;
    const static int N_def = 10;
    const static int option_def = 0; // 0 - modified Ogata (default)
    constexpr static double Q_def =
        1.; // a rough estimate where the maximum of the function f(x) is
  ...
  public:
    FBT(double _nu = nu_def, int option = option_def, int _N = N_def,
        double _Q = Q_def); // Constructor
    ~FBT();                 // Deconstructor
