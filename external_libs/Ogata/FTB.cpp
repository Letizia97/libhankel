// ###############################################################################
// # # #                Fast Bessel Transform (FBT) for TMDs # #     Zhongbo
// Kang, Alexei Prokudin, Nobuo Sato, John Terry                   # # Please
// cite ArXiv:1906.05949                              # #                  N is
// number of function calls                              # #                  nu
// is Bessel function order                                #
// ###############################################################################
...
void FBT::acknowledgement() {
    ...
    std::cout << "#                                                              "
                 "               #"
              << std::endl;
    std::cout << "#                Fast Bessel Transform (FBT) for TMDs          "
                 "               #"
              << std::endl;
    std::cout << "#     Zhongbo Kang, Alexei Prokudin, Nobuo Sato, John Terry    "
                 "               #"
              << std::endl;
    std::cout << "#                   Please cite Kang:2019ctl                   "
                 "               #"
              << std::endl;
    std::cout << "#                  nu is Bessel function order                 "
                 "               #"
              << std::endl;
    std::cout << "#                  option = 0,1,2  (modified, unmodified, "
                 "fixed h) Ogata      #"
              << std::endl;
    std::cout << "#                  N is number of function calls               "
                 "               #"
              << std::endl;
    std::cout << "#                  Q initial guess where function has maximum  "
                 "               #"
              << std::endl;
    std::cout << "###############################################################"
                 "################"
...
void FBT::citation() {
    std::cout << "###############################################################"
                 "################"
              << std::endl;
    std::cout << "#                     Thank you for using FBT!                 "
                 "               #"
              << std::endl;
    std::cout << "# Please, cite Kang:2019ctl if used for a publication          "
                 "               #"
              << std::endl;
    std::cout << "###############################################################"
                 "################"
...
FBT::FBT(double _nu, int _option, int _N, double _Q) {
    ...
    for (size_t i = 0; i < maxN; i++) {
        ...
        Jp1.push_back(
            gsl_sf_bessel_Jn(nu + 1.,
                             M_PI * xi[i])); // The functions gsl_sf_bessel_Jn and gsl_sf_bessel_Yn
                                             // return the result of the Bessel functions of the
...
double get_psi(double t) { return (t)*tanh(M_PI / 2. * sinh(t)); };
...
// double f_for_ogata
double f_for_ogata(double x, std::function<double(double)> g, double q) { return g(x / q) / q; };
...
double FBT::get_hu(std::function<double(double)> f, double q) {
    ...
    if (hu >= 2.) {
        hu = 2.;
        //    std::cerr<< "Warning: Number of nodes is too small N = " << this->N <<
        //    std::endl;
    }
