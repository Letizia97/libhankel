 ...
 * @note Computes an approximation to the nth positive zero of the Bessel
 * function of the first kind, for the special cases where the order is 0 or 1
...
double bessel_j_zero(int nzero, double order) {
    ...
    if (nzero < 1) {
        fprintf(stderr, "nzero, that is the index of the zero to be computed, "
    ...
    if (nzero < 7) {
        ...
        if (order == 0) {
            t = 0.0682894897349453 + 0.131420807470708 * beta * beta +
                0.0245988241803681 * pow4(beta) + 0.000813005721543268 * pow6(beta);
            b = beta + 1.16837242570470 * pow3(beta) + 0.200991122197811 * pow5(beta) +
                0.00650404577261471 * pow7(beta);
            j_zero = beta + t / b;
            return j_zero;

        } else if (order == 1) {
            t = -0.362804405737084 + 0.120341279038597 * pow3(beta) +
                0.0439454547101171 * pow4(beta) + 0.00159340088474713 * pow6(beta);
            b = beta - 0.325641790801361 * pow3(beta) - 0.117453445968927 * pow5(beta) -
        ...
        if (order == 0) {
            ...
            j_zero = beta + 1.0 / (8.0 * beta) - 124.0 / (1536.0 * pow3(beta)) +
                     120928.0 / (491520.0 * pow5(beta)) - 401743168.0 / (220200960.0 * pow7(beta));
            return j_zero;

        } else if (order == 1) {
            beta = (nzero + 0.25) * M_PI;
            j_zero = beta - 3.0 / (8.0 * beta) + 36.0 / (1536.0 * pow3(beta)) -
                     113184.0 / (491520.0 * pow5(beta)) + 1951209.0 / (220200960.0 * pow7(beta));
            return j_zero;
        }
    }
    return 0;
...
/**
 * @brief Computes sum from 1 to n of s(i) using Padé approximant implemented
 * with continued fraction expansion; see Z. Naturforschung, 33a, 402-417, 1978.
 *
...
double qwe_Chave(double nu, form_factor_f f, double r, void *f_params, double *output,
    ...
    if (!s) {
        // Allocation failed
        fprintf(stderr, "Failed to allocate internal variables in "
